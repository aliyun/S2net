...
class SwinSphDecoderNet(nn.Module):
    def __init__(self, cfg, pretrained=False):
        ...
        # compute healpixel
        self.neighbor_maps_all_levels = compute_hp_neighmap([128, 64, 32, 16, 8, 4])
        self.vit_encoder = SwinTransformer(pretrain_img_size=cfg.BACKBONE.PRETRAIN_RES,
        ...
        self.decoder = SphDecoder(cfg=cfg, neighbor_maps=self.neighbor_maps_all_levels,
        ...
        if pretrained:
            ...
            if cfg.TRAIN.PRETRAINED_MODEL == '':
                ...
                logger.info(f"Use pretrained model: {cfg.TRAIN.PRETRAINED_MODEL}")
        else:
...
class EffnetSphDecoderNet(nn.Module):
    def __init__(self, cfg, pretrained=False):
        ...
        basemodel = torch.hub.load('rwightman/gen-efficientnet-pytorch', basemodel_name, pretrained=pretrained)
        # Remove last layer
        logger.info('Removing last two layers (global_pool & classifier) in efficient net')
    ...
    def forward(self, x):
        ...
        enc_feat0, enc_feat1, enc_feat2, enc_feat3, enc_feat4 = \
            enc_feat[4], enc_feat[5], enc_feat[6], enc_feat[8], enc_feat[11]
