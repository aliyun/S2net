...
from .util_helper import precompute_pixelization_maps, precompute_position_encoding
...
class SphConv2d(nn.Module):
    def __init__(self, in_dims, out_dims, k, neigh_map, groups=1, bias=True):
        ...
        self.conv = nn.Conv1d(in_dims, out_dims, kernel_size=k**2, stride=k**2, groups=groups, bias=bias)
...
class SphPosEmbedding(nn.Module):
    ...
    def __init__(self, dim, pos_enc):
        ...
        #self.pos_emb = SphConv2d(3, dim, k=3, bias=False, neigh_map=neigh_map)
        self.pos_emb = nn.Conv2d(3, dim, 1)
        #self.pos_embedding = nn.Parameter(torch.randn(dim, 1, size))
...
class SphGSA(nn.Module):
    ...
    def forward(self, fmap):
        ...
        b, n, x, y, h = *shape, self.heads  # x = 1
        # windows number
        w = y // (k**2)
        q = self.to_q(fmap)
        fmap = rearrange(fmap, 'b n x (y p1 p2) -> (b x y) n p1 p2', p1=k, p2=k)
        kv = self.to_kv(fmap)
...
def make_skip_connection(in_dim, out_dim, pix_index_maps=None):
    ...
    skip_con1 = nn.Sequential(SphPixelization(pix_index_maps[0]),
                              nn.Conv2d(in_dim[0], out_dim1, 1),
                              nn.ReLU(False))
    skip_con2 = nn.Sequential(SphPixelization(pix_index_maps[1]),
                              nn.Conv2d(in_dim[1], out_dim2, 1),
                              nn.ReLU(False))
    skip_con3 = nn.Sequential(SphPixelization(pix_index_maps[2]),
                              nn.Conv2d(in_dim[2], out_dim3, 1),
                              nn.ReLU(False))
    skip_con4 = nn.Sequential(SphPixelization(pix_index_maps[3]),
                              nn.Conv2d(in_dim[3], out_dim4, 1),
                              nn.ReLU(False))
...
class ResidualConvBlock(nn.Module):
    def __init__(self, in_dim, activation, bn, neighbor_map):
        ...
        self.bn = bn
        self.groups = 1
        self.conv1 = SphConv2d(in_dim, in_dim, k=3, bias=not self.bn,
                               groups=self.groups, neigh_map=neighbor_map)
        self.conv2 = SphConv2d(in_dim, in_dim, k=3, bias=not self.bn,
                               groups=self.groups, neigh_map=neighbor_map)
...
class SphCrossAttFusionBlock(nn.Module):
    ...
    def forward(self, *xs):
        ...
        w = y // (k0 ** 2)
        q0 = self.to_q0(x0)
        x0_w = rearrange(x0, 'b n x (y p1 p2) -> (b x y) n p1 p2', p1=k0, p2=k0)
        kv0 = self.to_kv0(x0_w)
        kv0 = rearrange(kv0, '(b w) n x y -> b n x (w y)', w=w)
        k0, v0 = kv0.chunk(2, dim=1)
        q0, k0, v0 = map(lambda t: rearrange(t, 'b (h d) x y -> (b h) (x y) d', h=h), (q0, k0, v0))

        ############## for feature map x1 ##############
        w = y // (k1 ** 2)
        q1 = self.to_q1(x1)
        x1_w = rearrange(x1, 'b n x (y p1 p2) -> (b x y) n p1 p2', p1=k1, p2=k1)
...
class FeatureFusion(nn.Module):
    def __init__(self, enable_caf, dim, neighbor_map, activation=nn.ReLU(False),
                 bn=False, k0=4, k1=4, pos_enc=None):
        super(FeatureFusion, self).__init__()
        self.enable_CAF = enable_caf
        self.skip_add = None if self.enable_CAF else nn.quantized.FloatFunctional()
        self.resConfUnit2 = ResidualConvBlock(dim, activation, bn, neighbor_map=neighbor_map)
        self.resConfUnit1 = ResidualConvBlock(dim, activation, bn, neighbor_map=neighbor_map)
        self.fusionBlock = SphCAFBlock(dim, pos_enc, k0=k0, k1=k1) if self.enable_CAF else self.skip_add.add
...
class FeatureFusionBottom(nn.Module):
    def __init__(self, enable_caf, dim, neighbor_map, activation=nn.ReLU(False),
                 bn=False, k=1, pos_enc=None):
        super(FeatureFusionBottom, self).__init__()
        #self.resConfUnit = ResidualConvBlock(dim, dim_out, activation, bn, neighbor_map=neighbor_map)
        self.enable_CAF = enable_caf
        ...
        ) if self.enable_CAF else ResidualConvBlock(dim, activation, bn, neighbor_map=neighbor_map)
        self.up_sample = SphInterpolate(scale_factor=4)
...
class SphDecoder(nn.Module):
    def __init__(self, cfg, neighbor_maps, num_ch_enc, use_bn=False, img_size=(512, 1024)):
        super(SphDecoder, self).__init__()
        self.use_caf = cfg.MODEL.USE_CAF_FUSION
        self.pix_sample_index_maps = precompute_pixelization_maps(nsides=[32, 16, 8, 4],
                                                                  initial_img_size=[img_size[0] // 4, img_size[1] // 4])

        self.pos_encodings = precompute_position_encoding(nsides=[32, 16, 8, 4]) if self.use_caf else None
        self.ffBlock1 = FeatureFusion(cfg.MODEL.USE_CAF_FUSION, cfg.MODEL.DECODER_DIM, neighbor_map=neighbor_maps[32],
                                      bn=use_bn, k0=4, k1=4, pos_enc=self.pos_encodings[0] if self.use_caf else None)
        self.ffBlock2 = FeatureFusion(cfg.MODEL.USE_CAF_FUSION, cfg.MODEL.DECODER_DIM, neighbor_map=neighbor_maps[16],
                                      bn=use_bn, k0=2, k1=2, pos_enc=self.pos_encodings[1] if self.use_caf else None)
        self.ffBlock3 = FeatureFusion(cfg.MODEL.USE_CAF_FUSION, cfg.MODEL.DECODER_DIM, neighbor_map=neighbor_maps[8],
                                      bn=use_bn, k0=1, k1=1, pos_enc=self.pos_encodings[2] if self.use_caf else None)
        self.ffBlock4 = FeatureFusionBottom(cfg.MODEL.USE_CAF_FUSION, cfg.MODEL.DECODER_DIM, neighbor_map=neighbor_maps[4],
                                      bn=use_bn, k=1, pos_enc=self.pos_encodings[3] if self.use_caf else None)
        self.head = nn.Sequential(
            SphConv2d(cfg.MODEL.DECODER_DIM, cfg.MODEL.DECODER_DIM // 4, k=3, neigh_map=neighbor_maps[64]),
            nn.ReLU(True),
            SphInterpolate(scale_factor=4),
            SphConv2d(cfg.MODEL.DECODER_DIM//4, cfg.MODEL.DECODER_DIM // 8, k=3, neigh_map=neighbor_maps[128]),
            nn.ReLU(True),
            nn.Conv2d(cfg.MODEL.DECODER_DIM // 8, 1, kernel_size=1, stride=1, padding=0),
            nn.ReLU(True)
        )
