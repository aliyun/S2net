...
def compute_hp_neighmap(nsides):
    """ Precomputing the neighborhood indices table of each pixel on spherical surface.
        ...
        Args:
            nsides: the pixelization resolution, could be 4, 8, 16, 32, 64, 128,  which corresponds to pixle num 12 x nside^2
    ...
    for i in range(len(nsides)):
        ...
        logger.info(f'Compute hp neighbor map for nside: {nsides[i]}')
    return neighbours
...
def load_checkpoint_file(cfg, model, optimizer, lr_scheduler):
    logger.info(f"Resuming from {cfg.TRAIN.RESUME}....................")
