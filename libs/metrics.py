"""
The script implements evaluations including error metrics and accuracy metrics.
We keep them almost same as Unifuse(https://github.com/alibaba/UniFuse-Unidirectional-Fusion)
"""
...
@torch.no_grad()
def compute_depth_metrics(gt, pred, mask=None, median_align=False, min_depth=0.1, max_depth=10.0):
    if mask is None:
        mask = gt > 0
...
class Evaluator(object):
    ...
    def compute_eval_metrics(self, gt_depth, pred_depth, mask, max_depth=10.0):
        """
        ...
        abs_, abs_rel, sq_rel, rms, rms_log, log10, a1, a2, a3 = \
            compute_depth_metrics(gt_depth, pred_depth, mask, self.median_align, max_depth=max_depth)
